...
import newStock from '../Card/newstock.png';
...
const ModelProductCard = ({product}) => {
  ...
  return (
    <div>
        { (product?.category === "Furniture" || product?.category === "Decoration")? (
             (
                <motion.div
            layout
            variants={fadeUp}
          ...
          >
            <img className='newStock' src={newStock} alt='' />
            <Link to={`/product/${product?._id}`} className='flex-1'>
              <div  >
              { (product?.model) ? (
                  <Modelview url={product?.model} photo={product?.image} />
                )
              :(
